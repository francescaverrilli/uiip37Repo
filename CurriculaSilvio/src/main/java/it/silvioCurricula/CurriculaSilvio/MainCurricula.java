package it.silvioCurricula.CurriculaSilvio;

import java.util.Scanner;

import it.silvioCurricula.CurricolaSilvio.dao.impl.DefaultCandidatoDao;
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 1:
				...
				System.out.println("Dammi il codice del nuovo candidato");
				int codice = Integer.parseInt(input.nextLine());
				c1.setCodice(codice);

				System.out.println("Dammi il cognome del nuovo candidato");
				String cognome = input.nextLine();
				c1.setCognome(cognome);

				System.out.println("Dammi il nome del nuovo candidato");
				String nome = input.nextLine();
				c1.setNome(nome);

				System.out.println("Dammi la via della residenza del nuovo candidato");
				String via = input.nextLine();
				c1.setViaResidenza(via);

				System.out.println("Dammi il CAP della città di residenza del nuovo candidato");
				String cap = input.nextLine();
				c1.setCapResidenza(cap);

				System.out.println("Dammi la citta di residenza del nuovo candidato");
				String citta = input.nextLine();
				c1.setCittaResidenza(citta);

				System.out.println("Dammi la data di nascita del nuovo candidato");
